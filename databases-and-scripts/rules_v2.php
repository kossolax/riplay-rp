...
<?php
	...
	if( $_GET['action'] == "addQuestion" ) {
		if( isset($_POST['id']) ) {
			$id = intval($_POST['id']);
			mysql_query("DELETE FROM `site_nopyj_question` WHERE `id`='".$id."';") or die(mysql_error());
			mysql_query("DELETE FROM `site_nopyj_reponses` WHERE `idQuestion`='".$id."';") or die(mysql_error());
		}
