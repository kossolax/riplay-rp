...
public Action LoadSapin(Handle timer, any none) {
	...
	char query[2048];
	g_bLoading = true;
	Format(query, sizeof(query), "SELECT x, y, z FROM `rp_sapin` ORDER BY id ASC");
...
public Action SapinLoop(Handle timer, any none) {
	...
	if( amount > 0 && GetClientCount(true) >= 6 ) {
		int rand = Math_GetRandomInt(0, amount - 1);
		Entity_GetAbsOrigin(stack[rand], pos);
		amount = Math_GetRandomInt(3, 6);
		for (int i = 0; i <= amount; i++) {
			pos2 = pos;
			pos2[0] += Math_GetRandomFloat(-40.0, 40.0);
			pos2[1] += Math_GetRandomFloat(-40.0, 40.0);
			pos2[2] += 64.0;
			ServerCommand("rp_zombie_die %f %f %f", pos2[0], pos2[1], pos2[2]);
		}
	}
	
	float time = 120.0 - GetClientCount(true) * 1.5;
	CreateTimer(Math_GetRandomFloat(10.0, time)/2.0, SapinLoop);
}
