...
public int eventItemMenu(Handle p_hItemMenu, MenuAction p_oAction, int p_iParam1, int p_iParam2) {
	if (p_oAction == MenuAction_Select) {
		...
		if (GetMenuItem(p_hItemMenu, p_iParam2, szMenuItem, sizeof(szMenuItem))) {
			...
			if( g_bIsInCaptureMode && g_iServerRules[rules_ItemsDisabled][rules_Enabled] == 1 && g_iServerRules[rules_ItemsDisabled][rules_Target] == id && GetZoneBit(GetPlayerZone(p_iParam1)) & BITZONE_PVP && GetZoneBit(GetPlayerZone(p_iParam1)) == 200 && GetZoneBit(GetPlayerZone(p_iParam1)) == 201 && GetZoneBit(GetPlayerZone(p_iParam1)) == 315) {
