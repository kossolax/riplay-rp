...
void PrintHUD(int client, char[] szHUD, int size) {
	...
	if( g_iUserData[client][i_Dette] > 0 ) {
		...
		if( rp_GetClientJobID(client) == 1 && GetClientTeam(client) == CS_TEAM_CT && rp_GetZoneInt(rp_GetPlayerZone(client), zone_type_type) != 1 ) {
			Format(szHUD, size, "%s%T %s %T\n", szHUD, "HUD_Job", client, g_szJobList[ job ][job_type_name], szHUD, "Comico_Cops", client);
		}
		else if( rp_GetClientJobID(client) == 1 && GetClientTeam(client) == CS_TEAM_T && rp_GetZoneInt(rp_GetPlayerZone(client), zone_type_type) != 1 ) {
			Format(szHUD, size, "%s%T %s %T\n", szHUD, "HUD_Job", client, g_szJobList[ job ][job_type_name], szHUD, "Comico_Uncops", client);
		}
		else { 
