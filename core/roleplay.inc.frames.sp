		...
		if ( g_bUserData[i][b_isConnected] ) {
			...
			if( IsPlayerAlive(i) ) {
				...
				if( GetVectorDistance(fNow, g_fSuccess_last_move[i]) > 50 && Math_GetRandomInt(0, 1) ) {
					...
					if( !g_bUserData[i][b_GameModePassive] && jobID == 31 && Math_GetRandomInt(1, 200) == 42 || g_iUserData[i][i_Mp] > 0 && Math_GetRandomInt(1, 200) == 42 || g_iUserData[i][i_Abonne] > 0 && Math_GetRandomInt(1, 200) == 42 ) {
