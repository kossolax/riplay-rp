		...
		if ( g_bUserData[i][b_isConnected] ) {
			...
			if( IsPlayerAlive(i) ) {
				...
				if( GetVectorDistance(fNow, g_fSuccess_last_move[i]) > 50 && Math_GetRandomInt(0, 1) ) {
					...
					if( !g_bUserData[i][b_GameModePassive] && jobID == 31 && Math_GetRandomInt(1, 200) == 42 || !g_bUserData[i][b_GameModePassive] && rp_GetClientInt(client, i_Donateur) >= 1 && rp_GetClientInt(client, i_Donateur) <= 10 && Math_GetRandomInt(1, 200) == 42 ) {
