...
void updateBankCost(int client) {
	...
	for (int i = 0; i < max; i++) {
		...
		if( itemID == ITEM_JETONROUGE || itemID == ITEM_JETONBLEU )
			continue;
		if( StrEquals(g_szItemList[itemID][item_type_job_id], "51") || StrEquals(g_szItemList[itemID][item_type_job_id], "31") || StrEquals(g_szItemList[itemID][item_type_job_id], "211"), "rp_item_raw") == 0 )
			continue;
