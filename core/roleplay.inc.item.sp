...
void updateBankCost(int client) {
	...
	for (int i = 0; i < max; i++) {
		...
		if( itemID == ITEM_JETONROUGE || itemID == ITEM_JETONBLEU )
			continue;
		if( StrEqual((g_szItemList[itemID][item_type_job_id], "51") || StrEqual(g_szItemList[itemID][item_type_job_id], "31") || StrEqual(g_szItemList[itemID][item_type_job_id], "211"), "rp_item_raw") == 0 )
			continue;
