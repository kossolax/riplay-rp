...
void SynFromWeb() {
	static char base[] = "SELECT `money`, `bank`, `job_id`, `group_id`, `steamid`, `pseudo`, `steamid2`, `jail`, `raison`, `id`, UNIX_TIMESTAMP(`timestamp`) as `date`, `itemid`, `itemAmount`, `itemToBank`, `xp`, `jetonpass`, `jetonabo` FROM `rp_users2` ";
...
void LoadUserData(int Client) {
	...
	if(!IsFakeClient(Client)) {
		...
		Format(query, sizeof(query),
			"%s `allowed_dismiss`, `artisan_spe`, `abonne`, `jetonabo`, `jetonpass`, `bonusmp`, `bonuscoffre`, `bonusmairie` FROM `rp_users` WHERE `steamid` = '%s';", query, SteamID); 
...
public void LoadUserData_2(Handle owner, Handle hQuery, const char[] error, any Client) {
	...
	if (SQL_FetchRow(hQuery)) {
		...
		g_iUserData[Client][i_Bonuscoffre] = SQL_FetchInt(hQuery, 69);
		g_iUserData[Client][i_Bonusmairie] = SQL_FetchInt(hQuery, 70);
