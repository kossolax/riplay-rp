...
public Action Cmd_Cop(int client) {
	...
	if (( job == 104 || job == 105 || job == 106 || job == 107 ) && rp_GetZoneInt(zone, zone_type_type) != 101) {  // Juge formation, proximité et majistra dans le tribu
		ACCESS_DENIED(client);
	}
...
public Action Cmd_Tazer(int client) {
	...
	if (IsValidClient(target)) {
		...
		if (reward >= 0) {
			...
			TazerCooldown(client, 0.5);
		}
	}
	return Plugin_Handled;
}

void TazerCooldown(int client, float base) {
	int job = rp_GetClientInt(client, i_Job);
	float time = 10.0;
	...
	CreateTimer(time*base, AllowStealing, client);
}
